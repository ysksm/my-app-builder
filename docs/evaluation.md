...
| 3 | 2026-06-17 | 4.2 / 5 | ライブデータ層を実装し A/B を底上げ。A 4.0→4.5（データソース＋クエリ＋ライブ fetch＋`{{}}`式＋コンポーネント間連携＝ToolJet の核に到達）、B 4.0→4.5（データ層スライス1〜2） |

---

## スコアカード（Rev 3 / 2026-06-17）

| 観点 | スコア | Rev1比 | 一言 |
|---|:--:|:--:|---|
| A. プロダクト価値・ポジショニング | 4.5 | ↑0.5 | コード生成型の差別化＋**ライブデータ層が到達**（データソース/クエリ/`{{}}`/連携） |
| B. 機能網羅性 | 4.5 | ↑0.5 | UI/モデル/ロジック/RT/5FW/連携＋**データソース・クエリ・式バインド** |
...
**総合: 4.2 / 5** — D/F/I/J を底上げ後、ライブデータ層で A/B を 4.5 へ。残る上積み余地は C（生成コード整形/他 FW 深さ）と G（実ツール検証ゲート）、各観点の「残」項目。

### Rev 3 で実施（データ層）
- **slice1**: REST データソース＋クエリのモデル/コマンド/UI（🗄 データ）、テーブルの `queryRef` ライブ fetch 生成（`shared/data/queries.tsx`: useQuery + QueryTable）。
- **slice2a**: `{{ queries.<name>.<field> }}` 式バインド（text/見出し）→ `lookup(__scope, …)` にコンパイル。
- **slice2b**: コンポーネント間連携。`ComponentNode.name` ＋ 名前付き入力が `setVar` で value を公開、`{{ name.value }}` を `useScope` で購読（`shared/data/scope.tsx`、useSyncExternalStore）。
- すべて**実ビルド green ＋ 実行モードで live 動作確認**（クエリ表示・入力→テキストのリアクティブ反映）。式は安全のためドットパス限定。
- **残（データ層の続き）**: イベント起動クエリ（ボタン→run）、書き込み系（mutation）、table.selectedRow 等の公開変数拡充、他 FW への展開。
