...
- **FR-PAGE-07 画面サイズ指定**: ページごとに幅・高さを**固定 / 最小 / 最大 / 自動**で指定できる。ビルダーのキャンバスにリアルタイム反映(WYSIWYG)し、生成物にも `page-screen` ラッパーの inline style として出力する(全フレームワーク React/Vue/Svelte/Remix 共通、中央寄せ)。コマンド層(updatePage)経由なので GUI/MCP パリティを持つ
...
- **FR-GEN-07 フレームワーク generator のプラグイン化**: コード生成を「中立ドキュメントモデル → フレームワーク別 generator」の構造として明確化する。第1弾は React(現行実装)。将来 Vue / Svelte 等を追加する場合も、ドキュメントモデル・ヘッドレス仕様(FR-GUI-08)・トークン・I/F モデル・ルール/フローには手を入れず generator の追加のみで対応できることを設計上の不変条件とする(他の4系統と同じ「中立表現 + アダプタ」の構図)
...
| NFR-07 | 拡張性 | プラグイン機構を5系統で統一的に提供: ①UI パーツ(ヘッドレス仕様 + レンダラ実装) ②スタイル emitter ③I/F アダプタ(TypeSpec / OpenAPI…) ④通信コネクタ(REST / WS / Modbus…) ⑤フレームワーク generator(第1弾 React、将来 Vue / Svelte 等)。いずれも「中立な内部表現 + アダプタ」の構図 |
...
| M6: 拡張 | 継続 | プラグイン機構の公開、**OpenAPI アダプタ**、追加コネクタ(MQTT / OPC-UA 等)、BE 雛形生成の拡充、コラボレーション強化、**tailwind emitter + 既存 Tailwind 資産の取り込み**、**MCP Phase 2(エディタとの双方向同期)**、**別フレームワーク generator の PoC(FR-GEN-07 の実証)**、**デモモード(サンプル投入・ガイドツアー・シナリオ自動再生、§7.4)** |
