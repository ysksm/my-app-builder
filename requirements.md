...
| M3: モデル & I/F | +3ヶ月 | DDD モデルデザイナー(M3a 完了)、**中立 I/F モデル + アダプタ SPI**、TypeSpec アダプタ(双方向)、中立モデルからのコード生成(FE クライアント/モックサーバー)、CRUD 画面雛形導出 |
| M3.5: ロジック定義 | +2ヶ月 | **カスタムコード保護(FR-GEN-05)**、ルールエディタ(クロスフィールド/状態遷移 → ドメイン+UI 両生成)、ユースケースフローエディタ(application 層生成)、ドメインサービス契約定義 + スタブ/mock 生成(§4.3) |
